...
wordcloud(Fakenews$label, max.words = 20, scale = c(4, 0.8), colors= c("indianred1","indianred2","indianred3","indianred"))
wordcloud(News$label, max.words = 20, scale = c(4, 0.8), colors= c("lightsteelblue1","lightsteelblue2","lightsteelblue3","lightsteelblue"))
```
...
cleanCorpus <- rawCorpus %>% tm_map(toUpper) %>% tm_map(removeNumbers)
cleanCorpus <- cleanCorpus %>% tm_map(tolower) %>% tm_map(removeWords, stopwords()) %>% tm_map(removePunctuation,ucp = FALSE)
```
...
Error 1: Error in line 34, read.exl has to be read.csv
Error 2 & 3: Error in line 42 & 43, %> has to be %>%
Error 4: Error in line 48, Fakenews$label has to be Fakenews$text
Error 5: Error in line 49, News$label has to be News$text
Error 6: Error in line 66, removePunctuation,ucp = FALSE needs to be TRUE
Error 7: Error in line 121, findFreqTerms(1) needs to be findFreqTerms(5)
Error 8: Error in line 144, laplace = 0 needs to be laplace = 1
